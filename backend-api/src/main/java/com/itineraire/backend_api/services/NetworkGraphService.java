...
public class NetworkGraphService {
    ...
    // Change ces chemins si tes fichiers ne sont pas à la racine du projet
    ...
    private void loadStations() {
        try (BufferedReader br = new BufferedReader(new FileReader(STOPS_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                // 1. On ignore les lignes totalement vides (souvent à la toute fin du fichier)
                ...
                if (cols.length >= 4) {
                    ...
                    try {
                        // 2. Sécurité : si la case est vide, on met 0.0, sinon on convertit en Double
                        ...
                        // 3. Si le texte contient des lettres au lieu de chiffres, on affiche un petit message mais on NE COUPE PAS la lecture
                        System.err.println("⚠️ Coordonnées invalides ignorées pour la station : " + nom);
                    }
                }
            }
        } catch (IOException e) {
    ...
    private void loadEdges() {
        try (BufferedReader br = new BufferedReader(new FileReader(STOP_TIMES_FILE))) {
            String line;
            // On s'attend à ce que le fichier n'ait pas d'en-tête, ou on le passe si tu en as mis un
            // br.readLine(); // Décommente si tu as une ligne "trip_id,stop_id,stop_sequence" au début
                ...
                if (cols.length >= 3) {
                    ...
                    if (currentStation != null) {
                        ...
                        if (tripId.equals(currentTripId) && previousStation != null && !previousStation.equals(currentStation)) {
                            ...
                            // On vérifie que la liaison n'existe pas déjà (pour ne pas la recréer 100 fois par jour)
                            if (!networkGraph.containsEdge(previousStation, currentStation)) {
                                ...
                                // Poids par défaut = 1.0 (pour l'instant, l'itinéraire le plus court sera celui avec le moins d'arrêts)
    ...
    // Getter pour que le Membre 2 et 3 puissent récupérer le graphe
